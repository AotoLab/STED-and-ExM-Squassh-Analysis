function autoRun3_Analyze_3(folderN)% Filter outlier objects from msk image files; run analysis modules
...
xy_um = 0.017; % enter pixel size of original SIM image
z_um = 0.14; % enter slice depth of original SIM stack
...
presyn = [2]; pstsyn = [1 3]; %#ok<NBRAK> % % define labeling stragegy (pre/post channels)
%===============================================================
...
for sublp = 1:numsub
    ...
    filelist = dir(fullfile(subpath,'*mask.tif')); numFiles = size(filelist,1);
    fprintf('Processing folder %s\n',subname); synNum = str2double(subname(1:3));   
    csvlist = dir(fullfile(subpath,'*objFilterList.csv')); % TODO: create option to ignore if no object filtering (eg AP's STED data)
   %  try
    fltrTbl = importdata(fullfile(subpath,csvlist.name)); mskC = cell(1,numch);
    for chlp = 1:numch  % TODO - generalize channel number
        if numch == 2
            objs = fltrTbl.data(chlp,1:end);
%             objs = table2array(fltrTbl(pairs{1}(chlp),3:end));
        else
            objs = fltrTbl.data(chlp,1:end); 
        end
        objs(isnan(objs)) = [];
        objs = nonzeros(objs);
        % fltrC(chlp,:) = {objs}; clear objs
        msk = loadtiff(fullfile(subpath,filelist(chlp).name)); fileroot = filelist(chlp).name(1:end-4);
        % fltobjs = nonzeros(fltrC{chlp,1}); numflt = length(fltobjs);
        for ftlp = 1:length(objs)
            msk(msk==objs(ftlp)) = 0; 
        end
        % saveastiff - option overwrite,true?
        saveastiff(msk,fullfile(subpath,[fileroot,'_fltrd.tif'])); mskC(1,chlp) = {msk}; 
        clear fileroot msk objs
    end
   % try 
        if syVolFlg == 1
        ...
        clear fltrTbl ftlp csvlist mskC synSz chlp   
%     catch
%         synFlag = num2str(synNum);
%         warning(['Unable to perform syn',synFlag,' analysis: channel without objects.'])
%         errnum = errnum+1; errlog{errnum,1} = subname; %#ok<SAGROW>
%     end
%     catch
%     end
end %sublp
